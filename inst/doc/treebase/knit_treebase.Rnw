...
% \SweaveOpts{fig=TRUE, external=TRUE, cache=FALSE, message=FALSE, warning=FALSE, include=FALSE, tidy=FALSE, dev=pdf}
...
@
Summing the returned lists in each we find \Sexpr{sum(nature)} phylogenies in \emph{Nature} and \Sexpr{sum(science)} in \emph{Science}. Many more such exercises are possible, and several examples are included in the demo files provided with the package.  
...
The treebase API does not easily string 
<<findderryberry, eval=TRUE>>= 
...
<<treepar, cache=TRUE, message=FALSE, warning=FALSE, include=FALSE>>=
require(TreePar)
...
estimated_shifts <- bd.shifts.optim(x, sampling = c(1,1,1,1), 
