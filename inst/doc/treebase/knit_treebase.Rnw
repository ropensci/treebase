...
% \SweaveOpts{fig=TRUE, external=TRUE, cache=FALSE, message=FALSE, warning=FALSE, tidy=FALSE, dev='pdf'}
...
the research methods that make use of these phylogenies inferred from those programs are largely based in R, 
through over 30 dedicated packages that provide implementations of an ever-growing list of methods.  
...
@
For convenience, it is possible to load the copy of this data cached at the time the package was compiled.  
<<cache, echo=FALSE>>=
data(metadata)
@
...
\begin{figure}
\begin{center}
<<dates, fig.width=12, fig.height=6>>=
...
models <- list(             yule = pureBirth(tt),  
                     birth_death = bd(tt),     
                     yule_2_rate = yule2rate(tt),
      linear_diversity_dependent = DDL(tt),    
 exponential_diversity_dependent = DDX(tt),
         varying_speciation_rate = fitSPVAR(tt),  
         varying_extinction_rate = fitEXVAR(tt),  
...
@ 
and confirm the result presented in~\citet{Derryberry2011}, that the \Sexpr{best_fit} model is the best fit to the data.  
...
<<<treepar_yule, include=FALSE, cache=TRUE>>=
...
<<<treepar_birthdeath, include=FALSE, cache=TRUE>>=
...
\end{quote}
While several digital tools are available to make these kind of measurements without the calipers~\citep[\emph{e.g.}][]{Lauback2007},
