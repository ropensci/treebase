...
Ntaxa <- sapply(tree_metadata,`[[`, 'ntax') 
...
taxa <- data.frame(Ntaxa=as.numeric(unlist(Ntaxa)), meta, author=first_author)
ggplot(taxa) + geom_point(aes(dates, Ntaxa, col='author'), position = 'jitter')
