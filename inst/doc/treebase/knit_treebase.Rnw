...
% \SweaveOpts{fig=TRUE, external=TRUE, cache=FALSE, message=FALSE, warning=FALSE, tidy=FALSE, dev=pdf, echo=TRUE}
...
The R statistical environment has become a dominant platform for researchers applying these phylogenies to rapidly growing set of questions.
While the task of inferring phylogenies from genomic data remains dominated by dedicated compiled software such as MrBayes~\citep{Huelsenbeck2001b}, BEAST~\citep{Drummond2007}, RAXML~\citep{Stamatakis2006},
the research methods that make use of these phylogenies are largely based in R through dedicated packages.  
...
trait evolution~\citep{Butler2004, Paradis2004, Harmon2008, Hipp2010, Revell2011, Eastman2011}, 
community ecology~\citet{Webb2008, Kembel2010}, 
phyloclimatic modelling~\citep{Warren2008, Evans2009b}, 
simulation of phylogenies and character evolution~\citep{Harmon2008, Stadler2011a, Boettiger2012}, 
...
27 are listed on the taskview on Phylogenetic Methods of the Comprehensive R Archive Network (CRAN) 
and new methods are added every year.  

TreeBASE (\href{http://treebase.org}{http://treebase.org}) is an online repository of phylogenies~\citep{Sanderson1994b, Morell1996}.  
With the recent advent of mandatory data archiving in many of the major journals publishing phylognetics-based research~\citep[\emph{e.g.}][]{Fairbairn2010, Piwowar2011, Whitlock2010}, such resources can expect to become increasingly prominent.  
TreeBASE provides an application programming interface (API), that lets applications make queries to the database.  
...
For instance, the following command searches for phylogenies containing dolphin taxa,
...
by specifying no function argument we can get metadata from the entire database: 
...
Here are the top several contributing journals:
<<head_pub, comment=NA>>=
...
We can then look at the distribution of publication years for phylogenies depositied in TreeBASE,
color coding by publisher in Fig~\ref{fig:1}.  

\setkeys{Gin}{width=0.95\textwidth}
\begin{figure}
\begin{center}
<<dates, fig.width=7, fig.height=3>>=
...
<<<treestats>>=
data(treebase)
Ntaxa <- sapply(treebase,`[[`, 'Nnode') + 1
@

<<find_years>>=
studyid <- sapply(treebase,`[[`, 'S.id')
sid <- sapply(metadata, `[[`, 'identifier')
sid <- gsub(".*TB2:S(\\d*)", "\\1", sid)
matches <- sapply(studyid, match, sid)
years <- sapply(matches, function(i)  metadata[[i]]$date)
years[sapply(years, is.null)] <- NA
@

<<taxagrowth, fig.width=7, fig.height=4>>=
taxa <- data.frame(Ntaxa= Ntaxa, years = as.numeric(unlist(years)))
ggplot(taxa) + geom_point(aes(years, Ntaxa), position='jitter')
@
...
Imagine we are reading the recent phylogenetics paper,~\citet{Derryberry2011}. 
...
<<treepar, message=FALSE, warning=FALSE, include=FALSE>>=
require(TreePar)
## TreePar must first reformat the phylogeny data:
x<-sort(getx(derryberry), decreasing=TRUE)
# look for shifts occuring at times between 0 & 60 with interval of 5
# consider yule models, allow up to 3 shifts (4 different rates) 
estimated_shifts <- bd.shifts.optim(x, sampling = c(1,1,1,1), grid = 5, start = 0, end = 60, ME = FALSE, yule = TRUE)
## TreePar happens to return the model parameters in the second return element:
models <- estimated_shifts[[2]]
aic <- sapply(model, function(x) 2 * (length(x) - 1) + 2 * x[1] ) 
which.min(aic)
@


and confirm that the Yule 2-rate %\Sexpr{#which.min(aic)}-rate 
model is the best fitting by AIC score, 
even when allowing for up to four different shifts.  

\subsection{A new kind of meta-analysis}
...
Often referred to by their authors as meta-analyses, these approaches have focused on re-analyzing phylogenetic trees collected from many different earlier publications.  This is a more direct approach than the traditional concept of meta-analysis where statistical results from earlier studies are weighted by their sample size without actually repeating the statistical analyses of those papers, and avoids some of the statistical challenges inherent in summarizing results that may differ in methodology, etc.  
...
While potentially valuable, these researchers have often gone through heroic efforts simply to assemble these data sets from the literature.  As described in \citet{McPeek2007}, (emphasis added)
\begin{quote}
A PDF format file of each article was obtained, and a digital snapshot of the figure was taken in Adobe Acrobat 7.0. This image was transferred to a PowerPoint (Microsoft) file and printed on a laser printer. The phylogenies included in this study are listed in the appendix. \emph{All branch lengths were measured by hand from these  printed sheets using dial calipers.}
\end{quote}
for each of 163 phylogenies appearing in the literature. While several digital tools are now available to make these kind of measurements without the calipers, it is immensely easier and less error-prone to pull properly formatted phylogenies from the database for this purpose.   
...
The central question of many of meta-analyses mentioned above has been whether or not phylogenies show a changing rates of evolution. 
This question is imporant for several reasons.  %% ADD STUFF & CITE 
...
Like most comparative methods, this analysis will require ultrametric trees (branch lengths proportional to time, rather than to mutational steps). As most of these phylogenies are calibrated with branch length proportional to mutational step, we must time-calibrate each of them first.  This is a simple function 
...
@
 ...
 Applying this test to all of the currently available trees in TreeBASE,
<<gamma, echo=TRUE>>=
# gamma statistic
