...
\begin{document}
\begin{frontmatter}
  \title{\texttt{treebase}: explore, visualize, and integrate phylogenetic data in R}
...
through two separate interfaces.  The \texttt{OAI-PMH} interface 
...
For instance, one can search for phylogenies containing dolphin taxa, "Delphinus,"
...
deposited in TreeBASE using a separate API built on the OAI-PMH
protocol, an international web standard such data.  
...
Note that no single journal dominates the submissions, 
and taxa-specific publications as well as more broad-scope journals share the top ten spots. 
...
In addition to this information about the publications, we can obtain metadata about the phylogenies themselves, 
including the study identifier number of where they were published, the number of taxa in the tree, 
a quality score (if available), kind of tree (gene tree, species tree, or barcode tree) 
and whether the phylogeny represents a single or consensus type.  
<<tree_metadata_cache>>=
tree_metadata <- cache_treebase(only_metadata=TRUE)
@

For instance, we can summarize how the~\Sexpr{length(tree_metadata)} 
trees break out by kind or type (The \verb|xtable| command formats this as a table):
<<kind, results='tex'>>=
...
@
which returns just the metadata for the \Sexpr{length(genetrees)} gene trees in the database, from which we can look up the corresponding publication information.  
...
shifts not available in the earlier package. % models -> patterns
The \texttt{treebase} package can help us both verify the results presented
