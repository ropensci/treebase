...
@
which confirms that the \texttt{\Sexpr{names(best_no_of_rates[best_model])}} Yule \Sexpr{best_no_of_rates[best_model][[1]]}-rate  
...
replicating an analysis can become a matter of several minutes of casual investigation rather
than a more tedious correspondance to get access to the data.  While this example has focused
on a case involving a single tree for simplicity of presentation, this same script could be applied
to verify studies involving many phylogenies, or to apply the same checks across a collection of papers
using a particular question.  
