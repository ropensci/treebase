...
built on the OAI-PMH protocol, an international web standard for such data.
...
as we illustrate later.

A common task for the `treebase` package is assisting in data discovery by identifying potential datasets
that meet a given list of requirements.  For instance, if our analysis
requires consensus gene trees with 100 or more taxa appearing in the journals
_Nature_ or _Science_, we can quickly identify candidate phylogenies using
the `metadata` function, which combines information from both of the 
TreeBASE APIs.  
...
The fields provided are listed in Table II.  
...
From the length of this list we see that there are currently 
`r prettyNum(length(unique(meta[["Study.id"]])))` published studies in the database. Subsetting
from the metadata can be done using the standard R syntax.  For instance
to obtain a list of all the dates of publication & names of the journals
...
``` {r dates, cache=FALSE, fig.cap="Histogram of publication dates by year, with the code required to generate the figure."}
    library(ggplot2)
    ggplot(meta) + geom_bar(aes(date, fill = publisher)) + opts(axis.text.x=theme_text(angle=90, hjust=1))
````
...
Phylogenetic metadata includes features such as the number of taxa in the tree, a quality score (if available), 
...
We can summarize how these `r dim(meta)[1]` trees 
break out by kind or type  using the `phylo_metadata` function 
to extract the kind of tree
(gene/species/barcode) and type (single or consensus):


``` {r  kind }
 table(phylo_metadata("kind", phylo.md), phylo_metadata("type", phylo.md))
````
...
dumps are not available from treebase.org, this function has intentional delays to avoid
...
``` {r   taxagrowth, fig.cap="Combining the metadata available from publications and from phylogenies themselves, we can visualize the growth in taxa on published phylogenies. Note that the maximum size tree deposited each year is growing far faster than the average number."}
    ggplot(meta, aes(date, ntaxa)) + stat_smooth(aes(group = 1))
````
