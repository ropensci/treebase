...
across ecology and evolution. Once again the product of one field has
become the raw data of the next. Unfortunately, while the discipline of
...
package was to address this gap by providing how programmatic and
...
largely based in R. The R statistical environment [@RTeam2012] has
...
methods, incuding Java [@Maddison2011], MATLAB [@Blomberg2003]
and Python [@Sukumaran2010] and online interfaces
[@Martins2004].
...
``` {r   taxagrowth, fig.width=16, fig.height=6  }
...
````
At this point we have `r length(tt)` time-calibrated phylogenies over which we will apply the diversification rate analysis. 
...
``` {r   gammadist, caption="Distribution of the gamma statistic across phylogenies in TreeBASE. Strongly positive values are indicative of an increasing rate of evolution (excess of nodes near the tips), very negative values indicate an early burst of diversification (an excess of nodes near the root)."   }
qplot(gammas)
````
...
````
wherein we find that `r round(non_const, 2)*100` % of the trees can reject the constant-rates model at the 95% confidence level. 
...
submissions seen in Fig. [fig:1]. Accompanied by faster and more
