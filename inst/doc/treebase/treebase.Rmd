...
methods and user community in the R language.
...
download and perform a reproducible and systematic analysis using the rich set of tools
...
data in XML (extensible markup language) format, conforming the RSS1.0
...
Nexus file format and the more data-rich NeXML format [@Vos2012],
none of the R packages currently available for phylogenetic research 
are positioned to read these NeXML files.  As our `treebase` package seeks
to provide a bridge the repository and the R packages for phylogenetic analysis,
support for the NeXML formats must await greater integration of this format.
Meanwhile, the next version of the `treebase` package will provide extraction
of metadata information from the NeXML through XML parsing.  
...
to discuss such queries ahead of time.  Once run, the cache is saved
...
Though the cache included in the package will be updated during major package 
revisions.  The timestamp of the cache provided can be viewed in the help
file for the data object using the command `r ?treebase`.  
...
the database.  The fields provided by `metadata` are listed in Table II.  
...
Using the functions we introduce above, we provide a simple example that
...
to mutational step, we must time-calibrate each of them first. This function
